...
export default (props = {}) => {
  ...
  useEffect(() => {
    if(userInfo.userInfo && userInfo.userInfo.name){
      setUser(userInfo.userInfo.name)
    }
  }, [])
        ...
        <SimpleBar className={`collapse ${showClass} sidebar d-md-block bg-primary text-white`}>
          <div className="sidebar-inner px-4 pt-3">
            ...
            <Nav className="flex-column pt-3 pt-md-0">
              ...
              <NavItem title="Student"  link={ Routes.Student.path } icon={faUserPlus} />
              <NavItem title="Batch" link={ Routes.Batch.path } icon={faList} />
              <NavItem title="Course" link={ Routes.Course.path } icon={faList} />
